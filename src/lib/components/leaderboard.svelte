...
<div class="mb-5 w-[100%]">
	<div class="relative z-1 bg-deep-900 <lg:mb-5 <lg:h-65px">
		<img
			src="/bars.png"
			on:click={() => isHidden.update()}
			class="absolute w-0px <lg:w-40px mt-3 ml-3"
			alt=""
		/>
	</div>
	<div class="lg:mt-6 flex flex-col">
		<h1 class="text-5xl mb-3 text-white font-bold italic ml-auto mr-auto">
			{$_("leaderboard.leaderboard")}
		</h1>
		<div class="ml-auto mr-auto w-[90%] lg:w-[70%]">
			<div class=" bg-deep-500" />
			<div class="flex justify-between lg:text-xl bg-orange-500 rounded-xl p-4 lg:p-5 mb-3">
				{#each keys as key, i}
					<h1
						class="text-deep-900 {i !== 0 ? 'flex-1' : ''} {i === 1 ? 'ml-5' : ''} font-sans-bold"
					>
						{i === 0 ? "" : "|"}&nbsp{key}
					</h1>
				{/each}
			...
			<div>
					...
					{#each users.data.slice((currentPage - 1) * 10, currentPage * 10) as user, i}
						<div
							class="flex flex-row justify-between text-sm lg:text-l text-white  bg-deep{i % 2 === 0
								? ''
								: '-700'} rounded-xl p-4 lg:p-5 mt-2"
						>
							<h1 class="w-5">#{i + (currentPage - 1) * 10 + 1}</h1>
							<div class="ml-5 flex flex-row flex-1">
								<h1>| &nbsp</h1>
								<h1>
									{user.username.length > 19 ? `${user.name.substr(0, 16)}...` : user.username}
								</h1>
							</div>
				...
				{:else}
					<div class="flex flex-col justify-center items-center">
						<h1 class="text-white font-sans-bold lg:text-4xl">
							{$_("leaderboard.no_users_found")}
						</h1>
					</div>
				{/if}
			</div>
