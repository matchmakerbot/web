<script>
	...
	const userDataAndServerList = (async () => {
		const cookieValidation = await fetch(`https://${HOST_NAME}:8080/api/v1/auth/validatecookie`, {
			method: "GET",
			headers: {
		...
		const serverListRequest = await fetch(
			`https://${HOST_NAME}:8080/api/v1/guilds/getguildsuserandbotisin`,
			{
				method: "GET",
				headers: {
					"Content-Type": "application/json",
		...
		const userDataRequest = await fetch(`https://${HOST_NAME}:8080/api/v1/users/getuserdata`, {
			method: "GET",
			headers: {
				"Content-Type": "application/json",
			},
